...
class KAdapterModel(PreTrainedModel):
    ...
    def __init__(self, 
        ...
        if config is None:
            config = KAdapterConfig.from_adapter_configs(basemodel.config, [adapter.config for adapter in adapters], head.config)
        else:
        ...
        if head is None:
            head_config = AutoConfig.for_model(config.head.head_type, config.head)
            head = AutoModel.from_config(head_config)
